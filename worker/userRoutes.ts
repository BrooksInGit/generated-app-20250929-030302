...
import { processPdfsForComparison } from "./comparison-service";
/**
    ...
    app.post('/api/compare', async (c) => {
        try {
            ...
            if (!files || files.length === 0) {
                return c.json({ success: false, error: 'No files uploaded.' }, { status: 400 });
            }
            const comparisonData = await processPdfsForComparison(files);
            return c.json({ success: true, data: comparisonData });
        } catch (error) {
